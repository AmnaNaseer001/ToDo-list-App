...
while (Project) {
    let input = await inquirer.prompt([
        {
            ...
            choices: [
                chalk.magentaBright.bold.italic("Enter" + chalk.yellowBright.bold(' "New" ') + "to add in list"),
                chalk.magentaBright.bold.italic("Enter" + chalk.yellowBright.bold(' "List" ') + "to display list"),
                chalk.magentaBright.bold.italic("Enter" +
    ...
    if (Array[0] === undefined) {
        ...
        console.log(chalk.redBright.bold.italic("                      NO Entries                          "));
        console.log(chalk.blueBright.bold.italic("\n==========================================================\n"));
    }
    if (input.select ===
        chalk.magentaBright.bold.italic("Enter" + chalk.yellowBright.bold(' "Delete" ') + "to delete from list")) {
        let deleted = await inquirer.prompt([
            {
                name: "task",
                type: "string",
                message: chalk.magentaBright.bold.italic("Enter index number you want to delete?"),
            },
        ]);
    ...
    if (input.select ===
        chalk.magentaBright.bold.italic("Enter" + chalk.yellowBright.bold(' "Quit" ') + "to close the application")) {
        break;
    }
    else {
        continue;
